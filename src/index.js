...
let loadGLBWithData = function () {
    loader.load("models/sdhiuasgiu.glb",
        (glb) => {
...
let movement = function () {
    ...
    if (Math.abs(gamepad.axes[1]) > 0.5) {
        move.x += (side_direction.x * gamepad.axes[1]);
        move.z += (side_direction.z * gamepad.axes[1]);
    }
...
let insert_text_panel = function (text) {
    ...
    text_panel.setAttribute("text", { "value": text, "height": "auto", "width": "auto", "align": "center", "color": "#e9e7e7"});
